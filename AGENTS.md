...
- 历史汇率对比：`remit compare --on <日期>` 用 ECB 当日公布的定盘价重跑同一
  走廊；仅 FX 层随日期变化，实时报价类网络在两侧同时排除，保证对比同源
...
- 控制台文案必须同时写入 `en.json` 与 `zh-CN.json`；新增说明模板或错误代码时
  必须补齐中文模板且占位符一致；来源登记表以英文条目为准，新增或修改条目时
  必须同步 `zh-CN.json` 中按证据编号登记的译文，且译文保留全部数字
