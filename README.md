...
- **Historical comparison:** `remit compare` prices one corridor at two ECB
  rate dates. Only the FX table moves between the runs, and providers that
  quote at request time are excluded from both sides, so the deltas isolate
  the rate regime rather than mixing in a provider-set difference.
- **Break-even analysis:** `remit breakeven` finds the amounts at which the
  best route changes — fixed fees dominate small transfers, FX spread
...
- **Quality:** Python 3.11-3.13 CI, strict pytest configuration, expanded Ruff
  rules, package-build validation, and 351 automated tests, including browser
...
- CLI runs resolve the FX source once at startup, so a long command sequence
  uses one rate table throughout. `remit serve --fx live` re-checks for a newer
  ECB publication every 30 minutes (`--fx-refresh-minutes` tunes or disables
  it); the rate date it displays is always the one routing is using.
